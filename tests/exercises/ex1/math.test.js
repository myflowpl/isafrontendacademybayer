...
describe('multiplyOneHaveBePositiveAndNonZero', () => {
    test('should return positive value when both are positivies', () => {
        const actual = multiplyOneHaveBePositiveAndNonZero(3,4);

        if (actual === 12) {
            return;
        } else {
            throw new Error('Assertion failed!, Should be 12')
        }
    });
    ...
    test('should return error when one value is 0', () => {
        try {
            ...
            if (e.message === 'Zero not allowed') {
                ...
                throw new Error('Assertion failed!, Should be Zero not allowed')
            }
        }
    });
    ...
    test('should return error when both value are negative', () => {
        try {
            multiplyOneHaveBePositiveAndNonZero(-3,-1);
        } catch (e) {
            if (e.message === 'Both numbers have to be positive.') {
                return;
            } else {
                throw new Error('Assertion failed!, Should be Both numbers have to be positive.')
            }
        }
    });
});
